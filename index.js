...
app.get("/filter", (req, res) => {
  ...
  if (rol_id) {
    pool.query(
      'SELECT Concat(??, " ", ??, " ", ??) as nombre, id FROM usuarios where rol_id = ?',
      [nombreArray[0], nombreArray[1], nombreArray[2], rol_id],
      (err, results) => {
        if (err) {
          return res.json(err);
        }
    ...
    pool.query(
      "SELECT nombre, id from proyectos where activated = 0",
      [nombreArray[0], nombreArray[1], nombreArray[2], table],
      (err, results) => {
        if (err) {
          return res.json(err);
        }
    ...
    if (nombreArray.length > 1) {
      pool.query(
        `SELECT Concat(??, " ", ??, " ", ??) as nombre, id FROM ??`,
        [nombreArray[0], nombreArray[1], nombreArray[2], table],
        (err, results) => {
          if (err) {
            return res.json(err);
      ...
      pool.query(
        `SELECT ??, id FROM ?? ${isDisabled == false && "where activated = 1"} `,
        [nombre, table],
        (err, results) => {
          if (err) {
            return res.json(err);
  ...
  pool.query("call getUserWithRole(?)", [user], (err, results) => {
    try {
      ...
      if (results[0][0].activated == 0) {
        return res
          .status(400)
          .json({ msj: "not users found", activated: false });
      }
      ...
      bcrypt.compare(psw, results[0][0].password, (err, isMatch) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ msj: "Error comparing passwords", error: true });
        }
        console.log(isMatch);
        if (isMatch) {
  ...
  pool.query(
    ...
    (err, results) => {
      ...
      if (results.length == 0) {
        return res
          .status(400)
          .json({ msj: "not users found", userError: true });
      }

      if (results[0].correo_electronico != email) {
        return res
          .status(400)
          .json({ msj: "Emails doent match", emailError: true });
      }

      pool.query(
        "UPDATE usuarios SET estado = 1 WHERE nombre = ?",
        [user],
        (err, results) => {},
      );
      ...
      bcrypt.hash(psw, saltRounds, (err, hash) => {
        if (err) {
          return res
            .status(500)
            .json({ msj: "Error hashing password", error: true });
        }
        ...
        pool.query(
          ...
          (err, results) => {
            if (err) {
              return res
                .status(500)
                .json({ msj: "Error updating password", error: true });
            }
            return res
              .status(200)
              .json({ msj: "Successfully Updated", correct: true });
          },
        );
      });
    },
...
function organizeAdminData(data) {
  ...
  data.forEach((item) => {
    if (item.Tipo === "Centro_de_Negocios") {
      const entity = organizedData.Entidad.find(
        (e) => e.Nombre === item.Entidad,
      );
      if (entity) {
        const itemData = {
          localId: item.localID,
          nombre: item.Centro_de_Negocio,
          activated: item.activated,
        };
    ...
    data.reduce(
      (
        ...
        if (VarianteID && VarianteNombre) {
          acc[TipoBonoID].subtipos.push({
            id: VarianteID,
            nombre: VarianteNombre,
            activated: ActivatedVariante,
          });
        }
...
app.post("/saveData/", async (req, res) => {
  ...
  const hasHeadOfHousehold = familyMembers.some(
  ...
  try {
    async function crearProyecto() {
      pool.query(
        "SELECT id FROM variantes_bono WHERE id = ?",
          ...
          try {
            ...
            pool.getConnection((err, connection) => {
              ...
              connection.beginTransaction((err) => {
                ...
                // Insert propietario
                connection.query(
                  "INSERT INTO propietarios (tipo_propietario_id, cedula) VALUES (?, ?)",
                  [
                    directionData.loteTipoIdentificacion == "pendiente"
                      ? null
                      : directionData.loteTipoIdentificacion,
                    directionData.loteIdentificacion == "pendiente"
                      ? null
                      : directionData.loteIdentificacion,
                  ],
                  (err, propietarioResult) => {
                    ...
                    // Insert lote
                    connection.query(
                      "INSERT INTO lotes (propietario_id, numero_plano_catastro, numero_finca, provincia, distrito, canton, senas_descripcion) VALUES (?, ?, ?, ?, ?, ?, ?)",
                      [
                        propietarioId,
                        directionData.numeroPlanoCatastro,
                        directionData.finca,
                        directionData.provincia,
                        directionData.distrito,
                        directionData.canton,
                        directionData.otrasSenas,
                      ],
                      (err, loteResult) => {
                        if (err) {
                          return connection.rollback(() => {
                        ...
                        connection.query(
                          ...
                          [
                            ...
                            newSubtipoSeleccionado,
                            loteId,
                            formDataAdmin.presupuesto == ""
                              ? null
                              : formDataAdmin.presupuesto,
                            formDataAdmin.avaluo == ""
                              ? null
                              : formDataAdmin.avaluo,
                            formDataAdmin.entidad,
                            formDataAdmin.entidadSecundaria == "pendiente"
                              ? null
                              : formDataAdmin.entidadSecundaria,
                            formDataAdmin.analistaEntidad == "pendiente"
                              ? null
                              : formDataAdmin.analistaEntidad,
                            formDataAdmin.analistaIPSUM,
                            formDataAdmin.fiscalAsignado == "pendiente"
                              ? null
                              : formDataAdmin.fiscalAsignado,
                            formDataAdmin.ingenieroAsignado == "pendiente"
                              ? null
                              : formDataAdmin.ingenieroAsignado,
                            formDataAdmin.arquitecto == "pendiente"
                              ? null
                              : formDataAdmin.arquitecto,
                            formDataAdmin.Promotor_Ipsum == "pendiente"
                              ? null
                              : formDataAdmin.Promotor_Ipsum,
                            formDataAdmin.apc,
                            formDataAdmin.cfia,
                            projectData.hasFIS,
                            formDataAdmin.constructor == "pendiente"
                              ? null
                              : formDataAdmin.constructor,
                          ],
                          (err, proyectoResult) => {
                            if (err) {
                              return connection.rollback(() => {
                            ...
                            connection.query(
                              "INSERT INTO familias (proyecto_id, tipo_miembro, nombre, apellido1, apellido2, cedula, tipo_cedula, ingreso, tipo_ingreso, otroIngreso, telefono, email, adulto_mayor, discapacidad, imagen_cedula) VALUES ?",
                              [familyValues],
                              (err) => {
                                if (err) {
                                  return connection.rollback(() => {
    ...
    return res
      .status(200)
      .json({ message: "Error al guardar el proyecto", ok: false });
  }
});
...
app.post("/updateUser", (req, res) => {
  ...
  pool.query(
    "UPDATE usuarios SET nombre = ?, apellido1 = ?, apellido2 = ?, rol_id = ?, correo_electronico = ? WHERE id = ?",
    [userName, lastName1, lastName2, roleId, email, id],
    (err, results) => {
      try {
        if (err) {
          console.log(err);
...
app.post("/addUser", (req, res) => {
  ...
  bcrypt.hash(process.env.DEFAULT_PASS, saltRounds, (err, hash) => {
    ...
    pool.query(
      "INSERT INTO usuarios (nombre, apellido1, apellido2, correo_electronico, rol_id, password) VALUES (?, ? ,?, ?, ?, ?)",
      [userName, lastName1, lastName2, email, roleId, hash],
      (err, results) => {
        try {
          if (err) {
...
app.post("/changeStatus", (req, res) => {
  ...
  pool.query(
    "UPDATE usuarios SET activated = ? WHERE id = ?",
    [newValue, id],
    (err, results) => {
      try {
        if (err) {
      ...
      try {
        ...
        pool.getConnection((err, connection) => {
          ...
          connection.beginTransaction((err) => {
            ...
            // Update propietario
            connection.query(
              "Update propietarios set tipo_propietario_id = ?, cedula = ? where id = ?",
              [
                directionData.loteTipoIdentificacion,
                directionData.loteIdentificacion,
                directionData.lote_id,
              ],
              (err, propietarioResult) => {
                if (err) {
                  return connection.rollback(() => {
                ...
                // Update lote
                connection.query(
                  "Update lotes set numero_plano_catastro = ?, numero_finca = ?, provincia = ?, distrito = ?, canton = ?, senas_descripcion = ? where id = ?",
                  [
                    directionData.numeroPlanoCatastro,
                    directionData.finca,
                    directionData.provincia,
                  ...
                  (err, loteResult) => {
                    ...
                    connection.query(
                      "Update proyectos set nombre = ?, descripcion = ?, grupo_proyecto_id = ?, tipo_bono_id = ?, variante_bono_id = ?, fecha_ingreso = CURDATE(), presupuesto = ?, avaluo = ?, entidad_id = ?, centro_negocio_id = ?, analista_asigna_entidad_id = ?, analista_asigna_ipsum_id = ?, fiscal_id = ?, ingeniero_id = ?, arquitecto_id = ?, promotor_interno_id = ?, codigo_apc = ?, codigo_cfia = ?, fis = ?, constructor_id = ? where id = ?",
                      [
                        projectName,
                        projectData.desc,
                        projectData.grupoSeleccionado,
                      ...
                      (err, proyectoResult) => {
                        ...
                        for (let i = 0; i < familyMembers.length; i++) {
                          ...
                          if (familyMembers[i].id) {
                            connection.query(
                              "Update familias set tipo_miembro = ?, nombre = ?, apellido1 = ?, apellido2 = ?, cedula = ?, tipo_cedula = ?, ingreso = ?, tipo_ingreso = ?, otroIngreso = ?, telefono = ?, email = ?, adulto_mayor = ?, discapacidad = ?, imagen_cedula = ? where id = ?",
                              [
                                ...
                                familyMembers[i].ingresos == ""
                                  ? null
                                  : familyMembers[i].ingresos,
                                familyMembers[i].tipoIngresos == ""
                                  ? null
                                  : familyMembers[i].tipoIngresos,
                                familyMembers[i].otroIngreso == "N/A"
                                  ? null
                                  : familyMembers[i].otroIngreso,
                                familyMembers[i].telefono,
                                familyMembers[i].email,
                                familyMembers[i].adultoMayor,
                                familyMembers[i].discapacidad,
                                familyMembers[i].cedulaFile
                                  ? familyMembers[i].cedulaFile
                                  : null,
                                familyMembers[i].id,
                              ],
                            ...
                            connection.query(
                              "Insert into familias (proyecto_id, tipo_miembro, nombre, apellido1, apellido2, cedula, tipo_cedula, ingreso, tipo_ingreso, telefono, email, adulto_mayor, discapacidad, imagen_cedula) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                              [
                                projectData.idProyecto,
                                familyMembers[i].tipoMiembro,
                                familyMembers[i].nombre,
...
app.post("/updateEtapa", (req, res) => {
  ...
  pool.query(
    "update proyectos set etapa_actual_id = ?, subetapa_actual_id = ? where id = ?",
    [etapa, subetapaFixed, id],
    (err, results) => {
      if (err) {
        console.log(err);
        return res.json(err);
...
app.post("/insertBitacora", (req, res) => {
  ...
  pool.query(
    "insert into entradas_bitacora (descripcion, color, usuario_id, proyecto_id, fecha_ingreso, tipo) values (?, ? ,?, ?, ?, ?)",
    [descripcion, color, usuario, proyecto, newDate, tipo],
    (err, results) => {
      if (err) {
        console.log(err);
        return res.json(err);
      ...
      pool.query(
        "update proyectos set estado_color = ? where id = ?",
        [color, proyecto],
        (err, results) => {
          if (err) {
            console.log(err);
...
app.post("/genericUpdate", (req, res) => {
  ...
  for (i = 0; i < claves.length; i++) {
    if (claves[i] !== "id") {
      pool.query(
        "Update ?? set ?? = ? where id = ?",
        [table, claves[i], valores[i], dataEdit.id],
        (err, results) => {
          if (err) {
            console.log(err);
            return res.json(err);
...
app.post("/deleteProyecto", (req, res) => {
  ...
  pool.query(
    "UPDATE proyectos SET activated = ? WHERE id = ?",
    [newStatus, id],
    (err, results) => {
      try {
        if (err) {
...
app.get("/getEmails", (req, res) => {
  ...
  pool.query(
    "select ?? from proyectos where id in (?)",
    [emails, idProyecto],
    (err, results) => {
      if (err) {
        console.log(err);
      ...
      pool.query(
        "select correo_electronico from usuarios where id in (?)",
        [ids],
        (err, results) => {
          if (err) {
            console.log(err);
      ...
      if (results.length == 0) {
        ...
        try {
          ...
          bcrypt.hash(newPassword, saltRounds, (err, hash) => {
            ...
            pool.query(
              "Update usuarios set password = ?, estado = 0 where correo_electronico = ?",
              [hash, email],
              (err, results) => {
                if (err) {
                  console.log(err);
...
app.post("/pushPrueba", (req, res) => {
  ...
  pool.query(
    "insert into tokenTest (token) values (?)",
    [token],
    (err, results) => {
      if (err) {
        return res.json(err);
...
app.get("/getNotisLeidas", (req, res) => {
  ...
  pool.query(
    "select * from notificaciones where usuario_id = ? and leido = 0 order by fecha_ingreso desc",
    [user_id],
    (err, results) => {
      if (err) {
        return res.json(err);
...
app.get("/getAllNotis", (req, res) => {
  ...
  pool.query(
    "select * from notificaciones where usuario_id = ? order by fecha_ingreso desc",
    [user_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
app.post("/deleteReaded", (req, res) => {
  ...
  pool.query(
    "delete from notificaciones where usuario_id= ? and leido = 1 ",
    [id],
    (err, results) => {
      if (err) {
        return res.json(err);
...
app.post("/insertNoti", (req, res) => {
  ...
  pool.query(
    "Insert into notificaciones(message, usuario_id, fecha_ingreso) values (?, ?, ?)",
    [message, user_id, newDate],
    (err, results) => {
      if (err) {
        console.error(err);
        return res.json(err);
