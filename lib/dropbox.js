...
const CLIENT_ID = process.env.DROPBOX_CLIENT_ID_FELIPE;
const CLIENT_SECRET = process.env.DROPBOX_CLIENT_SECRET_FELIPE;
const REFRESH_TOKEN = process.env.DROPBOX_REFRESH_TOKEN_FELIPE;
...
async function getAccessToken() {
  ...
  return cache.getOrSet("dropboxAccessToken", TOKEN_TTL_MS, async () => {
    ...
    if (!response.ok) {
      ...
      Sentry.captureMessage("[dropbox] no se pudo refrescar el access token", {
        level: "error",
