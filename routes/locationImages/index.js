...
router.get("/:projectId", async (req, res) => {
  ...
  console.log(
    `[GET /locationImages/:projectId] consultando imágenes de ubicación del proyecto ${projectId}`,
  );
  ...
  const imagesData = await db
    ...
    .catch((err) => {
      console.error(
        `[GET /locationImages/:projectId] no se pudo conseguir las imágenes de ubicación (proyecto ${projectId})`,
        err,
      );
      res.status(400).json({
